...
```bash
git clone -b spot_teleop --recurse-submodules [https://github.com/EESC-LabRoM/MoveitServoing.git](https://github.com/EESC-LabRoM/MoveitServoing.git) ws_moveit
...
First detect an ArUco marker to establish a reference frame

Track the user's wrist position using MediaPipe

Convert 2D detections to 3D space using depth data

Publish the wrist position as a TF transform that can be used by MoveIt
...
In the RViz interface, locate the "Planned Path" section in the left panel

Disable the "Loop Animation" option by unchecking the box

In the "Manipulator Group" section, change the selection from "End Effector" to "Manipulator"

Use the interactive marker (colored arrows and rings) to move the manipulator to a valid position

Click the "Plan and Execute" button to make the robot move to the desired position
...
For more details on the container configuration, see the Dockerfile